...
Supports two modes:
  1. Local dev: reads firebase-service-account.json file
  2. Production (Render/Cloud Run): reads FIREBASE_SERVICE_ACCOUNT env variable
     which contains the JSON content as a string
"""
...
def _get_credentials():
    # Mode 1: Environment variable (production — Render, Cloud Run)
    ...
    if os.path.exists(os.path.abspath(local_path)):
        print("[Firebase] Using service account from local file")
        return credentials.Certificate(os.path.abspath(local_path))

    raise FileNotFoundError(
        "No Firebase credentials found. Either set FIREBASE_SERVICE_ACCOUNT "
        "environment variable or place firebase-service-account.json in backend/"
    )
