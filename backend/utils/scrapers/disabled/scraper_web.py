"""
ARCHIVED: scraper_web.py — Web search + Dailymotion URL collector (disabled for YouTube MVP).

This module is archived and not currently used. It contains multi-platform scrapers
for Google Search, Bing, Dailymotion, and Twitter/X.

To re-enable these scrapers in a future phase:
1. Create plugin wrappers that inherit from ScraperPlugin
2. Register them in utils/scrapers/__init__.py
3. Set ENABLED_SCRAPERS=youtube,dailymotion,twitter in .env

Original functionality has been replaced by YouTubePlugin for the MVP.
"""
