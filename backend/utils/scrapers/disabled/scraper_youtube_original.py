"""
ARCHIVED: scraper_youtube.py — YouTube URL collector using yt-dlp (original implementation, now replaced by plugin).

This is the original non-plugin implementation of the YouTube scraper.
It has been replaced by utils/scrapers/youtube_plugin.py which uses the plugin architecture.

The original search_youtube() and scan_channel() functions have been incorporated into the YouTubePlugin class.
"""
...
def search_youtube(sport: str, keywords: str, max_results: int = 10) -> list[dict]:
    ...
    DEPRECATED: Use YouTubePlugin.search() instead.
    """
...
def scan_channel(channel_url: str, sport: str, max_videos: int = 10) -> list[dict]:
    ...
    DEPRECATED: Channel scanning functionality moved to YouTubePlugin for consistency.
    """
